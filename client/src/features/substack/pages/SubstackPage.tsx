...
import { ArticleCard } from "../../medium/components/ArticleCard"
  ...
  return (
    <Container className="py-16">
      <SectionHeading
        ...
        subtitle="Shorter, regular notes on things I'm learning and building."
      />
      ...
      <div className="mb-8 flex max-w-sm items-center gap-2">
        <SearchInput
          ...
          aria-label="Search Substack posts"
          className="flex-1"
        />
      ...
      {isError && (
        <p className="text-red-600 dark:text-red-400">
          Couldn't load Substack posts right now.
        </p>
      )}
      {data && articles.length === 0 && (
        <p className="text-slate-500 dark:text-slate-400">
          {debouncedSearch ? "No posts match your search." : "No posts published yet."}
        </p>
      )}
      {articles.length > 0 && (
